<div class='form-group {{ $header_group_class }} {{ $errors->first($name) ? 'has-error' : '' }}'
    ...
    <div class="{{ $col_width ?: 'col-sm-10' }}">
        @if ($value)
            ...
            ?>
                    <div class='hex' style='width: 103.92px;height: 120px;'>
                        <div class='hex-background' style='width: 99.92px;height: 116px; '>
                            <img title="Image For {{ $form['label'] }}" src='{{ $url }}' />
                            <div class="overlay">
                                @if (!$readonly || !$disabled)
                                <a  class="icon" onclick="if(!confirm('{{ cbLang('delete_title_confirm') }}')) return false"
                ...
                <?php else:?>
                <p><a href='{{ $url }}'> <img style='max-width:160px' class="main-profile-img"
                            title="Image For {{ $form['label'] }}" src='{{ $fileurl }}' /></a></p>
                <?php endif;
