<div class='form-group {{ $header_group_class }} {{ $errors->first($name) ? 'has-error' : '' }}'
    ...
    <div class="{{ $col_width ?: 'col-sm-10' }}">
        <div class="input-group">
            ...
            <input type="tel" placeholder="(06)4229-5748" {{ $required }} {{ $disabled }} {{ $readonly }}
                class="fone form-control" {{ $validation['max'] ? 'maxlength=' . $validation['max'] : '' }}
                pattern="^\(0\d{1}\)(?:\d{1}){4}-(?:\d{1}){4}$"
